...
type Budget={monthlyBudgetThb:number;usedThb:number;remainingThb:number;usedPercent:number;usdToThb:number;enabled:boolean};
...
export default function AIBudgetGuardTab({token}:{token:string}) {
  ...
  async function saveBudget(){
    if(!budget)return;
    const r=await fetch('/api/admin/ai/budget',{method:'PATCH',headers:{'content-type':'application/json','x-neramit-admin':token},body:JSON.stringify({monthlyBudgetAmount:budget.monthlyBudgetThb,billingPeriodAnchor:1,enabled:budget.enabled,usdToThb:budget.usdToThb})});
...
function RuleEditor({rule,onChange,onSave}:{rule:Rule;onChange:(r:Rule)=>void;onSave:(r:Rule)=>void}) {
  return <div className="aiRule">
    ...
    <input type="number" value={rule.threshold} onChange={e=>onChange({...rule,threshold:Number(e.target.value)})}/>
